...
bool isValid(long cardNumber){
  ...
  for(int i=0; i<=sizeof(countedDigits); i++){
    ...
    if(countedDigits[i] >= 10){ // as per Luhn's Algorithm, we need to check if the number is higher than 10, because we are trying to get the digits and not the number itself, for example 15 should be 1 and 5

      while (countedDigits[i] > 0) {
